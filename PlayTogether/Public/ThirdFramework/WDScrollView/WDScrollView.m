...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context
{
    if (self.titleArr && [change[@"new"] integerValue] < self.titleArr.count) {
        ...
        NSAssert(self.currentIndex < self.imageArr.count, @"当前的位置大于数据的数量");
...
- (void)next
{
    ...
    self.titleLabel.text = self.titleArr[self.currentIndex];
    
    DDLog(@"当前的位置%ld-图片数组的数量%ld",self.currentIndex,self.imageArr.count);
    //配置下一页的图片数据
...
- (void)configWithObject:(id)obj andButton:(UIButton*)button
{
    if ([obj isKindOfClass:[NSURL class]]) {
        ...
        [button sd_setBackgroundImageWithURL:obj forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"big_default"]];
    }
    
    if ([obj isKindOfClass:[NSString class]]) {
        if ([obj hasPrefix:@"http"]) {
            [button sd_setBackgroundImageWithURL:[NSURL URLWithString:obj] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"big_default"]];
        }else{
            UIImage *image = [UIImage imageNamed:obj];
            if (image == nil) {
                image = [UIImage imageNamed:@"big_default"];
            }
    ...
    if ([obj isKindOfClass:[UIImage class]]) {
        if (obj == nil) {
            obj = [UIImage imageNamed:@"big_default"];
        }
...
{
    if (_timer == nil) {
        _timer = [NSTimer scheduledTimerWithTimeInterval:self.autoScrollTimeinterval target:self selector:@selector(next) userInfo:nil repeats:YES];
        _timer.fireDate = [NSDate dateWithTimeIntervalSinceNow:self.autoScrollTimeinterval];
    }
    return _timer;
}
